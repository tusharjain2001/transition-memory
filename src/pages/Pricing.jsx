...
const StepPill = ({ n, icon, label }) => (
  <div className="flex items-center gap-4">
    <div className="flex h-[52px] w-[52px] flex-shrink-0 items-center justify-center rounded-full border border-line bg-cream font-serif text-[24px] text-ink">
      {n}
    </div>
    <div className="flex items-center gap-5 rounded-xl border border-line bg-white px-6 py-4">
      <span className="text-brand">{icon}</span>
      <span className="whitespace-pre-line text-[14.5px] font-semibold leading-tight text-[#1f2a28]">{label}</span>
    </div>
  </div>
)
      ...
      <main>
        ...
        <section id="pricing" className="bg-page">
          <Container className="pb-8 pt-8 text-center">
            ...
            <p className="mt-3 text-[13.5px] text-muted">Prices exclude VAT where applicable.</p>
            ...
            <div className="mx-auto mt-8 grid max-w-[1000px] gap-6 text-center md:grid-cols-2">
              <div className="rounded-xl border border-line bg-cream px-8 pb-7 pt-6">
                <span className="inline-block rounded-full border border-brand px-6 py-1 text-[12.5px] font-bold uppercase tracking-[0.12em] text-brand">
                  Start here
                </span>
                <h2 className="mt-4 font-serif text-[28px] text-ink-soft">Founder Dependency Scan</h2>
                ...
                <p className="mx-auto mt-4 max-w-[400px] text-[14px] leading-[1.65] text-body">
                ...
                <p className="mx-auto mt-4 max-w-[320px] text-[13px] leading-[1.5] text-muted">
              ...
              <div className="overflow-hidden rounded-xl border-2 border-brand bg-white">
                <div className="bg-brand py-4">
                  <span className="inline-block rounded-full border border-white px-6 py-1 text-[12.5px] font-bold uppercase tracking-[0.12em] text-white">
                    Full transfer
                  </span>
                </div>
                <div className="px-8 pb-7 pt-5">
                  <h2 className="font-serif text-[28px] text-ink-soft">Verified Owner Transfer</h2>
                  <p className="mt-1 font-serif text-ink">
                    <span className="text-[32px]">From </span>
                    <span className="text-[46px] leading-none">£5,000</span>
                  </p>
                  <p className="text-[16px] text-body">per company + VAT</p>
                  <p className="mx-auto mt-3 max-w-[420px] text-[14px] leading-[1.65] text-body">
        ...
        <section className="bg-page">
          <Container className="pb-12 pt-2">
            ...
            <div className="mx-auto mt-5 max-w-[1000px] overflow-hidden rounded-xl border border-line bg-white">
              <div className="grid grid-cols-[1.4fr_1fr_1fr] border-b border-line text-center text-[14px] text-body">
                <div />
              ...
              {included.map(([label, a, b], i) => (
                <div
                  key={label}
                  className={`grid grid-cols-[1.4fr_1fr_1fr] items-center text-[14px] text-body ${i > 0 ? 'border-t border-line' : ''}`}
                >
        ...
        <section className="bg-brand text-white">
          <Container className="py-10 text-center">
            <h2 className="font-serif text-[30px]">What determines the full-transfer price?</h2>
            <div className="mx-auto mt-7 grid max-w-[1000px] gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {determines.map((d) => (
                <div key={d.title} className="rounded-xl border border-white/80 px-5 pb-6 pt-6">
                  ...
                  <h3 className="mt-3 whitespace-pre-line font-serif text-[22px] leading-[1.2]">{d.title}</h3>
                  <p className="mt-3 text-[14px] leading-[1.6] text-white/90">{d.body}</p>
                </div>
              ))}
            </div>
            <p className="mt-7 font-serif text-[16.5px]">
        ...
        <section className="bg-page">
          <Container className="pb-10 pt-8">
            ...
            <p className="mt-8 text-center text-[14.5px] text-body">
        ...
        <section className="bg-mint">
          <Container className="grid gap-8 py-9 lg:grid-cols-[1fr_1.4fr]">
            <div className="lg:pl-10">
              <p className="font-serif text-[19px] text-ink">After the owner steps back</p>
              <h2 className="mt-1 font-serif text-[30px] text-ink">Keep the Business Memory useful.</h2>
              ...
              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                {ongoing.map((o) => (
                  <Card key={o.title} className="flex flex-col items-center border-line-soft px-3 pb-5 pt-5 text-center">
                    ...
                    <p className="mt-4 whitespace-pre-line font-serif text-[15.5px] leading-[1.3] text-ink-soft">{o.title}</p>
                  </Card>
                ))}
              </div>
              <p className="mt-5 text-center text-[12.5px] font-bold uppercase tracking-[0.14em] text-brand">Priced to scope</p>
              <p className="mt-1 text-center text-[14px] text-body">There is no requirement to purchase an ongoing service.</p>
            </div>
          </Container>
        </section>
            ...
            <div className="mx-auto mt-6 grid max-w-[1000px] gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {notIncluded.map((t) => (
                <Card key={t} className="flex flex-col items-center px-4 pb-6 pt-6 text-center">
                  ...
                  <p className="mt-4 whitespace-pre-line font-serif text-[17.5px] leading-[1.3] text-ink-soft">{t}</p>
                </Card>
              ))}
            </div>
