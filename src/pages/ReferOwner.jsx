      ...
      <main>
        ...
        <section className="bg-page">
          <Container className="pb-8 pt-8 text-center">
            ...
            <p className="mx-auto mt-4 max-w-[600px] text-[14.5px] leading-[1.7] text-body">
            ...
            <div className="mt-7 flex flex-wrap items-center justify-center gap-12">
              {highlights.map((h) => (
                <div key={h.text} className="flex items-center gap-4">
                  ...
                  <span className="whitespace-pre-line text-left text-[13.5px] leading-snug text-body">{h.text}</span>
                </div>
              ))}
            </div>
          </Container>
        ...
        <section className="bg-[#f2f4f3]">
          <Container className="grid gap-8 py-8 lg:grid-cols-[1fr_2.1fr]">
            <div className="pt-1">
              <h2 className="font-serif text-[27px] text-ink">Before you refer</h2>
              <CheckList items={before} size="sm" className="mt-5 space-y-5 text-[13.5px]" />
              <div className="mt-7 flex gap-4 rounded-lg border border-[#f0dfc6] bg-[#fbf3e8] px-4 py-4">
                ...
                <p className="text-[12.5px] leading-[1.6] text-[#1f2a28]">
                  Do not include financial, personal, legally privileged or commercially sensitive information.
                </p>
              </div>
              <Link to={BOOK_PATH} className="mt-8 inline-flex items-center gap-2.5 border-b-2 border-brand pb-0.5 text-[13.5px] font-semibold text-brand">
            ...
            <Card className="px-7 pb-7 pt-6">
              ...
              <h2 className="mt-1 font-serif text-[27px] text-ink">Introduce the owner</h2>
              ...
              {submitted ? (
                <div className="mt-8 rounded-lg bg-mint-soft px-6 py-8 text-center">
                  ...
                  <h3 className="mt-4 font-serif text-[22px] text-ink">Referral received</h3>
                  <p className="mt-2 text-[13.5px] text-body">Thank you. We will review the referral and contact the people named.</p>
                </div>
              ) : (
                <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true) }} className="mt-5 space-y-5">
                  ...
                  <div className="space-y-3">
                    <label className="flex items-start gap-3 text-[13px] text-[#1f2a28]">
                  ...
                  <button type="submit" className="w-full rounded-md bg-brand py-3 text-[15px] font-semibold text-white shadow-sm transition hover:bg-brand-dark">
              ...
              {next.map((s, i) => (
                <div key={s.n}>
                  ...
                  <p className="mt-4 whitespace-pre-line leading-tight">
                    <span className="text-[15px] font-bold text-brand">{s.n}</span>
                    <span className="ml-2 font-serif text-[18px] text-ink">{s.title}</span>
                  </p>
                  <p className="mt-1.5 max-w-[210px] text-[13px] leading-[1.6] text-body">{s.body}</p>
                </div>
              ))}
            </div>
          </Container>
              ...
              <div className="rounded-xl border border-[#efe6d8] bg-cream px-6 pb-6 pt-6">
                <div className="flex gap-5">
                  ...
                  <div>
                    ...
                    <p className="font-serif text-ink"><span className="text-[38px] leading-none">£500</span><span className="ml-2 text-[16px]">+ VAT</span></p>
                ...
                <p className="mt-4 text-[13px] text-body">Credited against a full engagement if the company proceeds.</p>
              </div>
              <div className="rounded-xl border border-line-soft bg-[#eef3f0] px-6 pb-6 pt-6">
                <div className="flex gap-5">
                  ...
                  <div>
                    ...
                    <p className="font-serif text-ink"><span className="text-[24px]">From </span><span className="text-[38px] leading-none">£5,000</span></p>
                    <p className="font-serif text-[20px] text-ink">per company <span className="text-[15px]">+ VAT</span></p>
                  </div>
                </div>
                <p className="mt-4 text-[13px] leading-[1.6] text-body">A structured engagement to build the Business Memory, test successor readiness and deliver a final transfer report.</p>
        ...
        <section className="bg-brand text-white">
          <Container className="flex flex-col items-center gap-8 py-9 lg:flex-row lg:justify-center lg:gap-12">
            ...
            <div>
              <h2 className="font-serif text-[30px]">An introduction is not an instruction.</h2>
              <p className="mt-2 max-w-[440px] text-[15px] leading-[1.55]">The owner remains free to decline, ask questions or proceed only with the initial scan.</p>
            </div>
          </Container>
        </section>
            ...
            <div className="mx-auto max-w-[1000px] divide-y divide-line">
              {faq.map((f, i) => (
                <button key={f.q} type="button" onClick={() => setOpen((o) => o.map((v, j) => (j === i ? !v : v)))} className="flex w-full items-center justify-between gap-4 py-4 text-left">
                  <div>
                    <p className="text-[14px] font-semibold text-[#1f2a28]">{f.q}</p>
                    {open[i] && <p className="mt-1 text-[13px] text-body">{f.a}</p>}
                  </div>
          ...
          <Container className="pb-8">
            <div className="flex flex-col gap-6 rounded-xl border border-[#efe6d8] bg-cream px-7 py-6 lg:flex-row lg:items-center">
              ...
              <div className="flex-1">
                <h2 className="font-serif text-[27px] leading-tight text-ink">Prefer to let the owner contact us directly?</h2>
                <p className="mt-2 max-w-[440px] text-[13.5px] leading-[1.6] text-body">Share the Founder Dependency Scan page and allow them to review the service privately.</p>
              </div>
              <Link to={BOOK_PATH} className="inline-flex items-center gap-2.5 rounded-md border-2 border-brand px-7 py-2.5 text-[14px] font-semibold text-brand hover:bg-mint-soft">
