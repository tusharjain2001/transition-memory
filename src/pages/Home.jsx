...
function FaqCard({ f }) {
  ...
  return (
    <Card className="px-5 py-5">
      ...
      {open && <p className="mt-2 text-[13.5px] leading-[1.6] text-body">{f.a}</p>}
    </Card>
  )
}
      ...
      <main>
        ...
        <section className="bg-page">
          <Container className="grid items-start gap-10 pb-6 pt-8 lg:grid-cols-[1.05fr_1.5fr]">
            <div>
              ...
              <p className="mt-6 max-w-[460px] text-[15.5px] leading-[1.7] text-body">
              ...
              <p className="mt-7 inline-flex items-center gap-2.5 text-[14px] text-body">
        ...
        <section className="bg-cream">
          <Container className="py-8 text-center">
            <h2 className="font-serif text-[30px] text-ink">You may be ready to retire.&nbsp; Is the business ready to run without you?</h2>
            <p className="mx-auto mt-3 max-w-[560px] text-[14.5px] leading-[1.7] text-body">
            ...
            <div className="mx-auto mt-6 grid max-w-[1000px] gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {head.map((h) => (
                <Card key={h.title} className="flex flex-col items-center px-6 pb-7 pt-7 text-center">
                  ...
                  <h3 className="mt-4 font-serif text-[22px] text-ink-soft">{h.title}</h3>
                  <p className="mt-2 text-[14px] leading-[1.65] text-body">{h.body}</p>
                </Card>
              ))}
            </div>
              ...
              {journey.map((j, i) => (
                <div key={j.n} className="flex flex-col items-center text-center">
                  <div className="relative flex w-full items-center justify-center">
                    ...
                    <span className="relative z-10 flex h-[46px] w-[46px] items-center justify-center rounded-full border border-line bg-mint-soft text-[15px] font-semibold text-ink">{j.n}</span>
                  </div>
                  <div className="mt-5 text-brand">{j.icon}</div>
                  <h3 className="mt-2 text-[19px] font-semibold text-brand">{j.title}</h3>
                  <p className="mt-2 max-w-[230px] text-[14px] leading-[1.65] text-body">{j.body}</p>
                </div>
              ))}
            </div>
          </Container>
        ...
        <section className="bg-brand text-white">
          <Container className="grid items-center gap-10 py-10 lg:grid-cols-[1fr_1.15fr]">
            <div className="lg:pl-4">
              <h2 className="font-serif text-[32px]">Meet the Owner Memory Agent.</h2>
              <p className="mt-2 text-[16px]">The founder leaves. The business can still ask why.</p>
            ...
            <div className="rounded-xl bg-white px-5 pb-5 pt-4 text-[#1f2a28]">
              <div className="flex gap-4 border-b border-line pb-4">
                ...
                <div className="flex-1">
                  <div className="flex justify-between"><p className="text-[14px] font-semibold">You</p><span className="text-[11px] text-muted">10:28</span></div>
                  <p className="mt-0.5 text-[14.5px]">Why do we use this supplier?</p>
                </div>
              </div>
              <div className="flex gap-4 pt-4">
                ...
                <div className="flex-1">
                  <div className="flex justify-between"><p className="text-[14px] font-semibold">Owner Memory Agent</p><span className="text-[11px] text-muted">10:28</span></div>
                  <p className="mt-0.5 text-[14.5px] leading-[1.55]">
              ...
              <p className="mt-4 text-[13.5px] font-semibold">Sources</p>
              <div className="mt-1.5 flex flex-wrap items-center gap-2">
                <span className="rounded bg-mint px-3 py-1.5 text-[13px] text-brand">Founder interview</span>
                <span className="rounded bg-mint px-3 py-1.5 text-[13px] text-brand">Supplier review</span>
                <span className="ml-auto inline-flex items-center gap-1.5 text-[12.5px] text-body"><ShieldCheck size={16} className="text-brand" /> Supported by 2 sources</span>
              </div>
              <div className="mt-4 flex items-center justify-between rounded-md border border-line px-4 py-2.5 text-[14px] text-muted">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
              {produces.map((p) => (
                <Card key={p.title} className="flex flex-col items-center px-3 pb-6 pt-6 text-center">
                  ...
                  <p className="mt-4 whitespace-pre-line text-[15px] font-semibold leading-[1.3] text-brand">{p.title}</p>
                </Card>
              ))}
            </div>
            <p className="mt-5 text-center text-[14.5px] text-body">The Business Memory becomes the approved foundation for the AI agents.</p>
          </Container>
        </section>
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-3 md:divide-x md:divide-line">
                {groups.map((g) => (
                  <div key={g.title} className="flex gap-4 md:px-4">
                    ...
                    <div>
                      <h3 className="font-serif text-[18px] text-ink">{g.title}</h3>
                      <p className="mt-1 text-[14px] leading-[1.6] text-body">{g.body}</p>
                      {g.link && <ArrowLink to={g.to} className="mt-2 text-[14px]">{g.link}</ArrowLink>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Container className="pb-5">
            <Card className="grid gap-8 px-8 pb-7 pt-6 lg:grid-cols-[1.3fr_1fr_1fr] lg:divide-x lg:divide-line">
              <div>
                ...
                <h2 className="mt-3 font-serif text-[32px] text-ink">Founder Dependency Scan</h2>
                <p className="font-serif text-ink"><span className="text-[36px] leading-none">£500</span><span className="ml-2 font-sans text-[24px] text-body">+ VAT</span></p>
                <p className="mt-3 max-w-[380px] text-[14px] leading-[1.65] text-body">
              ...
              <div className="lg:pl-8">
                ...
                <p className="mt-4 text-[14px] leading-[1.6] text-body">The £500 fee is credited against a full TransitionMemory engagement if you proceed.</p>
              </div>
            </Card>
          </Container>
        </section>
            ...
            <div className="flex flex-col gap-6 rounded-xl border border-[#efe6d8] bg-cream px-8 py-6 lg:flex-row lg:items-center">
              ...
              <div className="grid flex-1 gap-x-8 gap-y-2 sm:grid-cols-2">
                {['Structured owner interviews', 'Transition readiness assessment', 'Business Memory', 'Owner Memory Agent setup', 'Successor testing'].map((t) => (
                  <span key={t} className="inline-flex items-center gap-2.5 text-[13.5px] text-body"><CheckCircle size={16} className="text-brand" />{t}</span>
                ))}
              </div>
        ...
        <section className="bg-page">
          <Container className="pb-8">
            <div className="rounded-xl border border-[#efe6d8] bg-cream px-8 py-8 text-center">
              <h2 className="font-serif text-[30px] text-ink">Your experience should become an asset the business keeps.</h2>
